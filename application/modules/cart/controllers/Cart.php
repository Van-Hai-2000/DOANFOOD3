...
class Cart extends MY_Controller
{
    ...
    function index()
    {
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            $this->prepare_flashmessage(get_languageword('Vui lòng đăng nhập để tiếp tục'), 2);    
            redirect(SITEURL);
        }
        ...
        if ($this->input->post('submit_type')=='save_order') {
            ...
            if ($current_time >= $from_time && $current_time <= $to_time) {
                ...
                $this->prepare_flashmessage(get_languageword('Đơn hàng không thể đặt trước xin vui lòng theo dõi thời gian của nhà hàng'), 2);
                redirect(URL_MENU);
            }
        }
